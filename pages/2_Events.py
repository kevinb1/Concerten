...
max_commas_rows = df_selection[df_selection['Lineup'].str.count(
    ',') == max_commas_count]
...
df_selection = df_selection.assign(
    Acts=df_selection['Lineup'].str.count(',') + 1)
...
df_chart['YearMonth'] = df_chart['Date'].str[:4] + \
    '-' + df_chart['Date'].str[5:7]
...
concerts_per_month_year = df_chart.groupby('YearMonth').agg(
    ...
    # You might want to customize how to handle multiple lineups
    Acts=('Acts', 'sum')
).reset_index()


# Convert 'YearMonth' to datetime for proper formatting
concerts_per_month_year['YearMonth'] = pd.to_datetime(
    concerts_per_month_year['YearMonth'])
