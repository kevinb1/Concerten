...
colA1.data_editor(df_concerts_selected, use_container_width=True,
                  hide_index=True, key="visited_concerts")

colA2.header("Collectables")
colA2.data_editor(df_collectables_selected,
                  use_container_width=True, hide_index=True, key="collectables")
...
if st.button(f"Aply Changes", type="primary"):
    if selected_table == "Visited Concerts":
        df_concerts_edited = edit_data(
            df_concerts_selected, edit_type, edit_input)
        df_concerts_edited.to_excel("Concerts.xlsx", index=False)
    else:
        df_collectables_edited = edit_data(
            df_collectables_selected, edit_type, edit_input)
